...
def parse_expression(state):
    ...
    print(f"  parse_expression: current token = '{state['current_token']['pattern']}' ({state['current_token']['token_name']})")

    expr = parse_simple_expression(state)
    if expr:
        print(f"  -> parsed as simple expression: {expr['node']}")
        return expr
    
    token_value = state['current_token']['pattern']
